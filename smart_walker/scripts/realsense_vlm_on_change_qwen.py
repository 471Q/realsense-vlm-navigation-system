...
# The element that caused the advisory is named by `hdsg_runtime.determine_authority` and recorded
# in every release as `evidence.action_binding_fact_ids` and `evidence.scene_binding_fact_ids`. The
# gate reads those two lists and the frozen release schema requires them.
#
# A second implementation of the same idea stood here until 25 August 2026: `identify_binding_fact`
# with `more_severe`, `ADVISORY_SEVERITY`, `_SEVERITY_TO_ADVISORY`, `BINDING_TIE_MARGIN_M`,
# `_LANE_NAMES` and `_LANE_STATUS_SEVERITY`, 190 lines that nothing called. It was the first design,
# left in place when the fact identifiers replaced it, and its docstring described how the
# transparency metric worked in the present tense: it named an `advisory_sources` record that exists
# nowhere in the tree and stated that scoring consulted it. Removing `binding_tie_margin` from the
# Fact Packet on 23 August, on the ground that no line of code read a 0.15 tie margin, took the
# record and left the code.
