...
def _call_vlm_with_fallbacks(endpoint: str, base_img: np.ndarray, args) -> str:
    ...
    # Qwen2-VL works well at 224
    size0 = int(getattr(args, 'image_size', 224) or 224)
...
def main():
    ap = argparse.ArgumentParser(
        description="Event-driven RealSense -> Qwen2-VL (trigger on risk/intent change)")

    # VLM parameters (defaults adapted for Qwen2-VL 3B)
    ap.add_argument('--endpoint', type=str, default='http://localhost:8080',
                    help='OpenAI-compatible VLM server base URL (llama.cpp)')
    ap.add_argument('--model', type=str, default='qwen2.5-vl-3b-instruct',
    ...
    # Trigger policy
    ap.add_argument('--min_interval_s', type=float, default=1.5,
    ...
    ap.add_argument('--image_size', type=int, default=224,
                    help='Resize square dimension before sending to VLM (Qwen2-VL works well at 224)')
                ...
                try:
                    ...
                    if 'mmproj' in msg or 'image input is not supported' in msg:
                        print("[hint] The llama.cpp server reported missing image support. "
                              "Start the server with a Qwen2.5-VL mmproj via -MmprojPath, e.g.:\n"
                              "  & .\\scripts\\start_llama_server.ps1 -ModelPath \"...qwen2.5-vl-3b-instruct-*.gguf\" -MmprojPath \"...qwen2.5-vl-3b-mmproj-*.gguf\" ...")
                except Exception:
                    pass
                last_vlm_ts = time.time()
                vlm_inflight = False
        ...
        while True:
            ...
            if args.show and latest_color is not None:
                ...
                try:
                    ...
                    pass
                cv2.imshow('VLM on change (Qwen2-VL)', vis)
